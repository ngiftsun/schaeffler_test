...
def build_regression(data,id):
    # get file name
    # Load file as array: 
    data = reject_outliers(data,ax=0,index=id, m = 2.3) 
    ...
    with open('linear_regression_1.pickle','wb') as f:
        pickle.dump(clf, f)
...
data = np.loadtxt('testData1.csv', delimiter=';', skiprows=1)
...
data_filtered.to_csv("test1_filtered.csv")
